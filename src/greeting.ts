...
import { themes, Color, ImageResolvable, toImage, getFontSize } from './util';
...
export async function drawCard(options: GreetingCardOptions): Promise<Buffer> {
    const w = 700, h = 250;
...
export async function welcomeImage(member: GuildMember, options: GreetingCardOptions = { }): Promise<Buffer> {
    const opts = { ...options }
    opts.text ??= { }
    opts.avatar ??= { }
...
export async function goodbyeImage(member: GuildMember, options: GreetingCardOptions = { }): Promise<Buffer> {
    const opts = { ...options }
    opts.text ??= { }
    opts.avatar ??= { }
